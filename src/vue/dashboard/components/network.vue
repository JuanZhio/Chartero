...
<script lang="ts">
import type { Point, PointMarkerOptionsObject, SeriesNetworkgraphNodesOptions } from 'highcharts';
import { Chart } from 'highcharts-vue';
import { defineComponent, type PropType } from 'vue';
import Highcharts from '@/highcharts';
import Tree from '@/tree';
import { helpMessageOption } from '@/utils';
import { createReadingKernelSnapshotForItem } from '$/history/kernel';
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            svgStr: Zotero.File.getResource('resource://chartero/icons/star.svg'),
        };
    },
    computed: {
        ...
        chartOpts() {
            return {
                plotOptions: {
                    networkgraph: {
                        layoutAlgorithm: {
                            ...
                            maxIterations: 100,
                            initialPositions: this.restorePosition ? layoutPosition : 'circle',
                        },
                    },
                    series: {
                        point: {
                            events: {
                                click: event => {
                                    ...
                                    // 单击加载更多
                                    else this.loadNode((event.point as any).id);
                                },
                            },
                        },
                    },
                ...
                chart: {
                    ...
                    events: {
                        load: ({ target: chart }) => {
                            ...
                            // 初始化颜色
                            const colors = (chart as unknown as Highcharts.Chart).options.colors;
                ...
                exporting: {
                    menuItemDefinitions: helpMessageOption(addon.locale.doc.network),
                },
    ...
    watch: {
        itemID(newID?: number, oldID?: number) {
            if (newID && !this.loading && this.show) {
                this.thisID = newID;
                if (this.visitedNodes.has(newID)) this.updateGraph(newID, oldID);
                else this.initGraph(newID);
            }
        },
        show(newVal: boolean) {
            if (!newVal) return;
            if (this.visitedNodes.has(this.topLevel!.id)) this.updateGraph(this.topLevel!.id, this.thisID);
            else this.initGraph(this.topLevel!.id);
            this.thisID = this.topLevel?.id;
        },
        loading(newVal) {
            const chartRef = (this.$refs.chartRef as typeof Chart)?.chart;
            if (newVal) chartRef?.showLoading();
            else chartRef?.hideLoading();
        },
    },
    methods: {
        async loadNode(id: string) {
            ...
            for (const key of item.relatedItems) {
                // 遍历相关条目
                const it = <Zotero.Item>Zotero.Items.getByLibraryAndKey(this.topLevel!.libraryID, key);
                if (!it || this.loadedNodes[it.id]) continue;
            ...
            if (data.length + nodes.length > 0) {
                const maxTime = Math.max(...this.graphNodes.map((node: any) => node.custom.time));
                for (const node of this.graphNodes) {
                    const size = maxTime ? Math.max(60 * ((node as any).custom.time / maxTime), 8) : 8;
                    node.marker = this.getMarker((node as any).id, size);
                }
            } else {
                const node = this.graphNodes.find(n => n.id == id);
                if (node) node.marker = this.getMarker(parseInt(id), node.marker?.radius);
            }
            ++this.refresh;
            this.loading = false;
        ...
        async collapseNode(node: Highcharts.Point) {
            ...
            if (!this.loadedNodes[id] || id == this.thisID) return;
            this.loading = true;
        ...
        removeNode(id?: number) {
            ...
            this.graphNodes = this.graphNodes.filter(node => node.id != id.toString());
            this.graphData = this.graphData.filter(edge => {
                // 删除相关边，并标记相关节点为未加载
                if (edge[0] == id.toString()) return (this.loadedNodes[edge[1]] = false);
                if (edge[1] == id.toString()) return (this.loadedNodes[edge[0]] = false);
                return true;
            });
        },
        createNode(item: Zotero.Item) {
            ...
            return {
                ...
                custom: {
                    ...
                    time: createReadingKernelSnapshotForItem(item).totalS,
                },
            } as SeriesNetworkgraphNodesOptions;
        },
        getMarker(id: number, size: number = 8): PointMarkerOptionsObject {
            const relatedColor = this.loadedNodes[id] ? this.thatColor : this.dim(this.thatColor),
        ...
        updateGraph(newID: number, oldID?: number) {
            ...
            for (const node of this.graphNodes)
                if (node.id == newID.toString() || node.id == oldID?.toString())
                    node.marker = this.getMarker((node as any).id, node.marker?.radius);
            ++this.refresh;
        },
    },
});
