...
export class PageRecord implements RecordBase {
    ...
    mergeJSON(json: _ZoteroTypes.anyObj): void {
        ...
        typeof json.t == 'number' && (this.totalSeconds = (this.totalSeconds ?? 0) + json.t);
        typeof json.s == 'number' && (this.selectText = (this.selectText ?? 0) + json.s);
        typeof json.u == 'object' && (this.userSeconds = { ...(this.userSeconds ?? {}), ...json.u });
    }
    toJSON() {
        return {
            t: this.totalSeconds,
    ...
    public get totalS() {
        return this.totalSeconds ?? (this.period && Object.values(this.period).reduce((sum, p) => sum + p));
    }
    ...
    public get lastTime(): number | undefined {
        if (!this.period) return;
        const arr = Object.keys(this.period);
        if (arr.length) return arr.reduce((result, timestamp) => Math.max(result, parseInt(timestamp)), 0);
    }
...
export class AttachmentRecord implements RecordBase {
    ...
    constructor(arg?: number | object) {
        ...
        this.schemaVersion ??= HISTORY_SCHEMA_VERSION;
    }
    mergeJSON(json: _ZoteroTypes.anyObj): void {
            ...
            if (Object.prototype.hasOwnProperty.call(json, key)) {
                if (key === 'pages') {
                    for (const page in json.pages)
                        if (parseInt(page) >= 0)
                            (this.pages[parseInt(page)] ??= new PageRecord()).mergeJSON(json.pages[page]);
    ...
    get lastPage() {
        return Object.keys(this.pages).reduce((last, page) => Math.max(last, parseInt(page)), 0);
    }

    public get firstTime() {
        const arr = this.pageArr.map(p => p.firstTime).filter(p => p) as number[];

        if (arr.length) return arr.reduce((result, timestamp) => Math.min(result, timestamp), Infinity);
    }

    public get lastTime() {
        const arr = this.pageArr.map(p => p.lastTime).filter(p => p) as number[];

        if (arr.length) return arr.reduce((result, timestamp) => Math.max(result, timestamp), 0);
    }
    ...
    public get userIDs() {
        return Array.from(
            new Set(this.pageArr.reduce((arr, page) => arr.concat(page.userIDs), [] as number[])),
        );
    }

    getUserTotalSeconds(userID: number): number {
        return this.pageArr.reduce((sum, page) => (page.getUserTotalSeconds(userID) ?? 0) + sum, 0);
    }
}
